...
### library(Require)
### require all except ok to library (9.1)
library("papaja")
library(tidyverse)
library(psych)
library(stats)
...
``` {r, PCLR-descriptives, echo=FALSE, warning=FALSE, fig.cap="(ref:PCLR-descriptives)"}
...
*reference tables 2 and 3* There is a significant relationship between predictor and outcome (*p* `r apa_p(summary(direct_model)$coefficients["PCLR_STAI_Trait_z", "Pr(>|t|)"], add_equals=T)`). However, this effect goes away when adding BPD as a mediator (*p* `r apa_p(mediation_result_total$z.avg.p, add_equals=T)`). This suggests that the presence of BPD acts as a mechanism through which the predictor influences the outcome. The significant, full mediation effect we observed suggests that a portion of the total effect of the predictor on the outcome is explained by the mediator (*p* ).
...
```{r STAI-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of STAI-Trait scores in sample", fig.pos="H"}
knitr::include_graphics("d2m-Psychopathy_files/figure-latex/STAI-descriptives-1.png")
```

```{r PAIBOR-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of PAI-BOR scores in sample", fig.pos="H"}
knitr::include_graphics("d2m-Psychopathy_files/figure-latex/PAI-descriptives-1.png")
```

```{r TAS-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of TAS scores in sample", fig.pos="H"}
