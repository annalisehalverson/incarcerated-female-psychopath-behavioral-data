...
## Participants
Collected over a four-year period, the present sample consists of 156 incarcerated females exhibiting varying levels of psychopathic and borderline tendencies. Participants range in age, from 20 to 53 (M = *reference*, SD = *reference*). Participants were randomly selected and subsequently informed of the nature of the study. Before screening, they were required to provide consent. Random selection was used to allow for a wide array of scores that could be systematically examined across the various facets of targeted measures.
...
```{r, STAI-descriptives, fig.show='hide'}
...
```{r, PAI-descriptives, fig.show='hide'}
...
```{r, TAS-descriptives, fig.show='hide'}
...
``` {r prelim-regression-output, results = 'hide'}
...
There is a significant relationship between predictor and outcome (*p* < 0.001 <!-- = `r summary(direct_model)$coefficients["PCLR_STAI_Trait_z", "Pr(>|t|)"]` -->). However, this effect goes away when adding BPD as a mediator (*p* = ). This suggests that the presence of BPD acts as a mechanism through which the predictor influences the outcome. The significant, full mediation effect we observed suggests that a portion of the total effect of the predictor on the outcome is explained by the mediator (*p* = ).
...
```{r STAI-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of STAI-Trait scores in sample"}
knitr::include_graphics("d2m-Psychopathy_files/figure-latex/STAI-descriptives-1.png")
```

```{r PAIBOR-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of PAI-BOR scores in sample"}
knitr::include_graphics("d2m-Psychopathy_files/figure-latex/PAI-descriptives-1.png")
```

```{r TAS-descriptives-appendix, echo=FALSE, out.width="100%", fig.cap="Distribution of TAS scores in sample"}
knitr::include_graphics("d2m-Psychopathy_files/figure-latex/TAS-descriptives-1.png")
```

```{r, echo=FALSE, results='asis'}
cat(readLines("prelim-regression-output.png"), sep = "\n")
```
