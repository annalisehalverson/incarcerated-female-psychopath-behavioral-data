...
library(papaja) # creates apa style manuscripts
library(tidyverse)
library(psych)
library(stats)
library(diagram) # creates mediation diagram
library(mediation) # runs mediation analyses
...
data <- c( 0, "", 0,
          0, 0, 0, 
          "", "", 0 ) # vector outlining boxes in space

M <- matrix ( nrow = 3 , ncol = 3 , byrow = TRUE , data = data ) # place into matrix
plot <- plotmat ( M , pos = c( 1 , 2 ), 
            ...
            box.prop = 0.5 ,
            curve = 0 ,
            arr.lwd = 1 ) # label and design each box
...
minimal_descriptives <- filtered_wide_data [ , c(
  "PAIBOR_Total_Score" , # bpd assessment scores
  "PCLR_Total_Score_Prorated" , # psychopathy assessment scores
  "TAS_Total_Score" , # alexithymia assessment scores
  "STAI_Trait_Anxiety" ) ] # anxiety assessment scores
assessment_names <- c(
  "PAI-BOR" , 
  "PCLR" , 
  "TAS" , 
...
stargazer ( minimal_descriptives , # generate n, mean, sd, min, max
          title = "Summary Table" , 
          header = FALSE , 
          type = 'latex' ,
          label = "tab:summary-table" ,
          digits = 1 ) # round to tenths place

```

```{r age-stats}

filtered_age_values <- filtered_wide_data$Age [ filtered_wide_data$Age != -1001 ] # remove missing age data
mean_excluded <- mean( filtered_age_values ) # calculate mean
sd_excluded <- sd( filtered_age_values ) # calculate sd
...
ggplot( data = filtered_wide_data , 
        aes( x = PCLR_Total_Score_Prorated ) ) +
  geom_histogram( fill = "#ca8dfd" , 
                  color = "#1c1c1c" , 
                  binwidth = 2 ) + # experiment with binwidth to determine how data best distributes
  scale_y_continuous( limits = c( 0 , 12 ) , # set appropriate y range given data
                      breaks = seq( 0, 12, 2 ) ) + # set label breaks
  ggtitle( "Histogram of Psychopathy" ) +
  labs( x = "PCL-R" , 
        y = "Count" ) +
  xlim( 0 , 40 ) # set appropriate x range given data
...
ggplot( data = filtered_wide_data, 
        aes( x = STAI_Trait_Anxiety ) ) +
  geom_histogram( fill = "#ca8dfd" , 
                  color = "#1c1c1c" , 
                  binwidth = 2 ) + # experiment with binwidth to determine how data best distributes
  scale_y_continuous( limits = c( 0 , 10 ) , # set appropriate y range given data
                      breaks = seq( 0 , 10 , 2 ) ) + # set label breaks
  ggtitle( "Histogram of Trait Anxiety" ) +
  labs( x = "STAI_Trait" , 
        y = "Count" ) +
  xlim( 20 , 80 ) # set appropriate x range given data
...
ggplot( data = filtered_wide_data , 
  ...
  xlim( 0 , 65 ) # set appropriate x range given data
...
ggplot( filtered_wide_data , 
        aes( x = PCLR_Total_Score_Prorated , # psychopathy on x
             y = TAS_Total_Score , # alexithymia on y
             color = STAI_Trait_Anxiety ) ) + # anxiety by color
  geom_smooth( method = "lm" , # association line
               color = "#1c1c1c" ) +
  geom_point( ) +
  scale_color_gradient( low = "#0000ff" , 
                        high = "#ff0000" ) + # scale dot color to demonstrate high/low anxiety score
  labs( x = "Psychopathy" , 
        y = "Alexithymia" , 
        title = "Relationship between Psychopathy, Anxiety, Alexithymia" , 
        color = "STAI-Trait" )

psytascorrelation <- corr.test(
  x = filtered_wide_data$PCLR_Total_Score_Prorated ,
  y = filtered_wide_data$TAS_Total_Score ) # correlate pclr & tas

staitascorrelation <- corr.test(
  x = filtered_wide_data$STAI_Trait_Anxiety ,
  y = filtered_wide_data$TAS_Total_Score ) # correlate stai & tas
...
ggplot( filtered_wide_data , 
  ...
  labs( x = "Psychopathy" , 
        y = "BPD" , 
        title = "Relationship between Psychopathy, Anxiety, BPD" , 
        color = "STAI-Trait" )

psybpdcorrelation <- corr.test(
  x = filtered_wide_data$PCLR_Total_Score_Prorated,
  y = filtered_wide_data$PAIBOR_Total_Score) # correlate pclr and pai

staibpdcorrelation <- corr.test(
  x = filtered_wide_data$STAI_Trait_Anxiety,
  y = filtered_wide_data$PAIBOR_Total_Score) # correlate stai and pai
...
filtered_wide_data$PAIBOR_Total_Score_z <-
  zscore( filtered_wide_data$PAIBOR_Total_Score ) # bpd assessment score

filtered_wide_data$PCLR_STAI_Trait_z <-
  zscore( filtered_wide_data$PCLR_STAI_Trait ) # psychopathy interaction assessment score

filtered_wide_data$TAS_Total_Score_z <-
  zscore( filtered_wide_data$TAS_Total_Score ) # alexithymia assessment score

filtered_wide_data$TAS_Factor1_Difficulty_Identifying_Feeling_z <-
  zscore( filtered_wide_data$TAS_Factor1_Difficulty_Identifying_Feeling ) # alexithymia assessment f1 score

filtered_wide_data$TAS_Factor2_Difficulty_Describing_Feelings_z <-
  zscore( filtered_wide_data$TAS_Factor2_Difficulty_Describing_Feelings ) # alexithymia assessment f2 score

filtered_wide_data$TAS_Factor3_Externally_Oriented_Thinking_z <-
  zscore( filtered_wide_data$TAS_Factor3_Externally_Oriented_Thinking ) # alexithymia assessment f3 score

```

``` {r prelim-regression-input, echo=FALSE, out.width='100%'}

po_path <- lm( TAS_Total_Score_z ~ 
                 PCLR_STAI_Trait_z , 
              data = filtered_wide_data ) # predictor-outcome
pm_path <- lm( PAIBOR_Total_Score_z ~ 
                 PCLR_STAI_Trait_z , 
              data = filtered_wide_data ) # predictor-mediator
mo_path <- lm( TAS_Total_Score_z ~ 
                 PAIBOR_Total_Score_z , 
              data = filtered_wide_data ) # mediator-outcome

```

``` {r prelim-regression-output, echo=FALSE, results='asis', warning=FALSE}

# create a table that outputs simple regression of predictor on outcome, predictor on mediator, and mediator on outcome. See Baron & Kenny (1986)
stargazer( po_path , 
           pm_path , 
           mo_path , 
           ...
           header = FALSE , 
           type = 'latex' , 
           dep.var.labels = c( "P-O Path" , 
                               "P-M Path" , 
                               "M-O Path" ) , 
           covariate.labels = c( "Psychopathy*Anxiety" , 
                                 "BPD" ) , 
           column.sep.width = "1pt" , 
           label = "tab:prelim-regression-output" , 
           out = "prelim-regression-output.png" )
...
direct_model <- lm( TAS_Total_Score_z ~ # tas total
                      PCLR_STAI_Trait_z , # pclr
                   data = filtered_wide_data )

direct_model1 <- lm( TAS_Factor1_Difficulty_Identifying_Feeling_z ~ # tas f1
                      PCLR_STAI_Trait_z , # pclr
                    data = filtered_wide_data )

direct_model2 <- lm( TAS_Factor2_Difficulty_Describing_Feelings_z ~ # tas f2
                      PCLR_STAI_Trait_z , # pclr
                    data = filtered_wide_data )

direct_model3 <- lm( TAS_Factor3_Externally_Oriented_Thinking_z ~ # tas f3
                      PCLR_STAI_Trait_z , # pclr
                    data = filtered_wide_data )
...
# create a table that outputs simple regression of psychopathy(predictor) on tas total,1,2,3(outcome)
...
mediator_model_total <- lm( PAIBOR_Total_Score_z ~ # mediator
                             PCLR_STAI_Trait_z , # predictor
                           data = filtered_wide_data )

model_total <- lm( TAS_Total_Score_z ~  # outcome
                    PCLR_STAI_Trait_z + # predictor
                     PAIBOR_Total_Score_z , # mediator
                  data = filtered_wide_data )

mediation_result_total <- mediate( mediator_model_total , # p-m
                                   model_total , # p-m-o
                                  treat = "PCLR_STAI_Trait_z" , # predictor
                                  mediator = "PAIBOR_Total_Score_z" , # mediator
                                  boot = TRUE, # bootstrapped
                                  sims = 500 ) # 500 simulations


mediator_model_tasf1 <- lm( PAIBOR_Total_Score_z ~ # mediator
                              PCLR_STAI_Trait_z , # predictor
                           data = filtered_wide_data )

model_tasf1 <- lm( TAS_Factor1_Difficulty_Identifying_Feeling_z ~ # outcome
                    PCLR_STAI_Trait_z + # predictor
                     PAIBOR_Total_Score_z , # mediator
                  data = filtered_wide_data )

mediation_result_tasf1 <- mediate( mediator_model_tasf1 , # p-m
                                   model_tasf1 , # p-m-o
                                  treat = "PCLR_STAI_Trait_z" , # predictor
                                  mediator = "PAIBOR_Total_Score_z" , # mediator
                                  boot = TRUE , # bootstrapped
...
mediator_model_tasf2 <- lm( PAIBOR_Total_Score_z ~ # mediator
                             PCLR_STAI_Trait_z , # predictor
                           data = filtered_wide_data )

model_tasf2 <- lm( TAS_Factor2_Difficulty_Describing_Feelings_z ~ # outcome
                    PCLR_STAI_Trait_z + # predictor
                     PAIBOR_Total_Score_z , # mediator
                  data = filtered_wide_data )

mediation_result_tasf2 <- mediate( mediator_model_tasf2 , # p-m
                                   model_tasf2 , # p-m-o
                                  treat = "PCLR_STAI_Trait_z" , # predictor
                                  mediator = "PAIBOR_Total_Score_z" , # mediator
                                  boot = TRUE , # bootstrapped
...
mediator_model_tasf3 <- lm( PAIBOR_Total_Score_z ~ # mediator
                              PCLR_STAI_Trait_z , # predictor
                           data = filtered_wide_data )

model_tasf3 <- lm( TAS_Factor3_Externally_Oriented_Thinking_z ~ # outcome
                    PCLR_STAI_Trait_z + # predictor
                     PAIBOR_Total_Score_z , # mediator
                  data = filtered_wide_data )

mediation_result_tasf3 <- mediate( mediator_model_tasf3 , # p-m
                                   model_tasf3 , # p-m-o
                                  treat = "PCLR_STAI_Trait_z" , # predictor
                                  mediator = "PAIBOR_Total_Score_z" , # mediator
                                  boot = TRUE , # bootstrapped
...
# create a table that outputs multiple regression of psychopathy(predictor) & bpd(mediator) on tas total,1,2,3(outcome)
...
```{r mediation-summaries, echo=FALSE, results='hide'}
...
```{r mediation-plot, results='hide', fig.cap="Direct and Mediated Effects Exploring TAS Factors", label='mediation-plot', out.width='100%'}

par( mfrow = c( 2 , 2 ) , mar = c( 3 , 4 , 3 , 4 ) + 0.1 )  # arrange plots in a 2x2 grid
plot( mediation_result_total ) # TAS total
mtext( "Total" , side = 3 , line = 1 , cex = 1.2 )
plot( mediation_result_tasf1 ) # TAS factor 1
mtext( "Factor 1" , side = 3 , line = 1 , cex = 1.2 )
plot( mediation_result_tasf2 ) # TAS factor 2
mtext( "Factor 2" , side = 3 , line = 1 , cex = 1.2 )
plot( mediation_result_tasf3 ) # TAS factor 3
mtext( "Factor 3" , side = 3 , line = 1 , cex = 1.2 )
...
BPD, as with all personality disorders, have “cultural histories” [@bjorklundNOMANLAND2006, p. 3]. Sociocultural factors will inevitably play a role in disease and diagnostic conditions, yet this hardly explains why a BPD diagnosis is considerably more common in women than in men. Future research should more deeply and centrally seek to elucidate what many actually be contributing to diagnostic disparity when it comes to gender and what may simply be a product of bias. It continues to remain possible that ASPD and BPD are simply gender-based constructions that serve to arrive at the same end point [@beauchaineMultifinalityDevelopmentPersonality2009]. On a grander scale, gender is not the only means for demonstrating diversity in psychopathological manifestation. Future research should consider other means of distinguishing psychopathy as well.   
The curious diversity of this mediation effect is certainly cause for future research. Emotion expression and regulation play crucial roles in daily interactions and interpersonal relationships. It is evident that abnormal emotional processing is central in the anatomies of both psychopathy and BPD. Research into this area will help to tailor essential treatment that elucidates earlier intervention points for how and when this concoction of maladaptive processes may contribute to an endgame of incarceration. This knowledge can guide the development of targeted interventions or strategies based on specific factors that are most influenced by the mediation process. Dialectical Behavioral Therapy [DBT\; @linehan1993dialectical] and DBT-inspired treatments have demonstrated preliminary yet promising results for incarcerated female populations [@perEvaluatingEffectivenessMindfulnessBased2020]. Regardless, these changes in significance emphasize the need for careful and nuanced interpretation, taking into account the specific characteristics and dynamics at play for each factor within the composite variables.  
...
---
nocite: |
  @lilienfeldMultidimensionalNaturePsychopathy2018
---
